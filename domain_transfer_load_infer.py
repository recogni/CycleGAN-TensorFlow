...
path_to_checkpoint_folder = 'checkpoint/horse2zebra_512/'

print ('Reading sample image...')
double_frame_in = np.zeros(shape=(1,512,512,6),dtype=np.float32)
sample_frame = np.expand_dims(imread('sample_inference/horse_512.jpg'),0)
sample_frame = np.asarray(sample_frame,dtype=np.float32)/127.5 - 1.0
double_frame_in[:,:,:,0:3] = sample_frame
print ('Shape of double_frame_in is: '+str(np.shape(double_frame_in))+' .')

with tf.Session() as session:
    print ('Loading meta graph of cycleGAN...')
    saver = tf.train.import_meta_graph(meta_graph_or_file=path_to_checkpoint_folder+'cyclegan.model-53002.meta')
    ...
    frame_inp = graph.get_tensor_by_name(name='real_A_and_B_images:0')
    ...
    domain_transfered_frame = session.run(frame_out,feed_dict={frame_inp: double_frame_in})
    print ('Infered single frame !')

    print ('Saving infered frame...')
    scipy.misc.imsave(name='sample_inference/horse_512_transfered.jpg',arr=domain_transfered_frame[0,:,:,0:3])
